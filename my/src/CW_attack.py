...
class Model():
    def __init__(self, model, img_size, num_channels, num_labels):
        self.model = model
        self.image_size = img_size
        self.num_channels = num_channels
        self.num_labels = num_labels
        self.encrypt = self.model.encrypt

    def predict(self, image):
        image = self.encrypt(image)
        inp = self.model.model.input
        outputs = self.model.model.layers[-2].output
        # print(outputs)
        func = K.function([inp], [outputs])
        layer_outs = func([image])[0][0]
...
with tf.Session() as sess:
    ...
    for i in range(len(adv)):
        ...
        print("Total distortion:", np.sum((adv[i] - image) ** 2) ** .5)
