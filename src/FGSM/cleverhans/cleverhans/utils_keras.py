...
class KerasModelWrapper(Model):
  ...
  def _get_logits_name(self):
    ...
    softmax_layer = self.model.model.get_layer(softmax_name)
  ...
  def get_layer_names(self):
    ...
    """
    layer_names = [x.name for x in self.model.model.layers]
    return layer_names
  ...
  def fprop(self, x):
    ...
    if self.keras_model is None:
      # Get the input layer
      new_input = self.model.model.get_input_at(0)
      ...
      # default.
      for x_layer in self.model.model.layers:
    ...
    if len(self.model.model.layers) == 1:
      outputs = [outputs]
